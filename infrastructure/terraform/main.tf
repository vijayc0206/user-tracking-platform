# User Tracking Platform - AWS Infrastructure
# Terraform Configuration for High Availability Deployment
...
terraform {
  ...
  # backend "s3" {
  #   bucket         = "user-tracking-terraform-state"
...
provider "aws" {
  ...
  default_tags {
    tags = {
      Project     = "user-tracking-platform"
...
# Local variables
locals {
  name_prefix = "utp-${var.environment}"
